...
function getToolLabel(toolName: string, args: any, state: string): string {
  if (toolName === "str_replace_editor" && args) {
    const file = args.path ?? "";
    switch (args.command) {
      case "create": return `Created ${file}`;
      case "str_replace": return `Edited ${file}`;
      case "insert": return `Edited ${file}`;
      case "view": return `Viewed ${file}`;
    }
  }
  if (toolName === "file_manager" && args) {
    switch (args.command) {
      case "rename": return `Renamed ${args.path} → ${args.new_path}`;
      case "delete": return `Deleted ${args.path}`;
    }
  }
  return state === "result" ? "Done" : "Working…";
}
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs font-mono border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{getToolLabel(tool.toolName, tool.args, tool.state)}</span>
                                  </>
                                ) : (
                                  <>
